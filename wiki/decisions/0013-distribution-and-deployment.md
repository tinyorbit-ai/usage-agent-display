# 0013 — Distribution & deployment: compiled daemon + vibe-realm server
...
("vibe-realm": PM2 + Doppler + Cloudflare Tunnel) on a VM.
...
2. **Server deploys via vibe-realm**, not a bespoke recipe. It carries a
   `packages/server/ecosystem.config.js` (PM2 app `usage`, `interpreter: bun`, Doppler
   `envVars`, `publicInternet.enabled`). The repo is listed in vibe-realm `repos.json`
...
- Reusing vibe-realm means PM2 lifecycle, Doppler secret injection, HTTPS, and the
...
- **Docker image for the server.** Heavier than the existing PM2/Bun path vibe-realm
...
- vibe-realm `repos.json` gains a `usage-agent-display` entry (owner commits there — that
